...
void silicio(){
    ...
    TGraphErrors *graph = new TGraphErrors("silicio.txt", "%lg %lg %lg %lg"); 
 
    TCanvas *c1 = new TCanvas("c1","Grafico I-V");

    TF1 *f = new TF1("f", "[0]*(exp(x/[1])-1)", 480, 750);
    ...
    graph->GetXaxis()->SetRangeUser(450, 750);
    graph->GetYaxis()->SetTitle("Corrente (mA)");
    graph->GetYaxis()->SetTitleOffset(0.7);
    graph->SetTitle("Grafico I-V Silicio");
